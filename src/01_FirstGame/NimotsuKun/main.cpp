...
int main() {
  ...
  // Visual
  // Studioから実行する人のために無限ループ。コマンドラインからはCtrl-Cで終えてください。
  while (true) {
    ;
  }
  return 0;
...
void initialize(Object *state, int width, int /* height */,
  ...
  while (*d != '\0') {  // NULL文字でない間
    Object
        t;  // 特に意味はないが使う回数が多い変数に私は良くtを使う。temporaryの略。たぶんよくない習慣だが、無駄に長い名前にして読みにくいのも困り物だろう。
    switch (*d) {
      case '#':
        t = OBJ_WALL;
        break;
...
void update(Object *s, char input, int w, int h) {
  ...
  if (s[tp] == OBJ_SPACE || s[tp] == OBJ_GOAL) {
    s[tp] = (s[tp] == OBJ_GOAL) ? OBJ_MAN_ON_GOAL
                                : OBJ_MAN;  // ゴールならゴール上の人に
    s[p] =
        (s[p] == OBJ_MAN_ON_GOAL)
            ? OBJ_GOAL
            : OBJ_SPACE;  // もともとゴール上ならゴールに
