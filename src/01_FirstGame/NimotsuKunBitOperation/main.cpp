...
class State {
 ...
 private:
  ...
  int mWidth;
  int mHeight;
  Array2D<unsigned char> mObjects;  // ビット演算するのでunsigned
                                    // char。ここまでけちらなくてもいいが。
};
  ...
  for (int i = 0; i < size; ++i) {
    ...
    switch (stageData[i]) {
      ...
      case 'o':
        ...
        break;
      case 'O':
        t = OBJ_BLOCK | OBJ_GOAL_FLAG;
        break;
      case '.':
        t = OBJ_SPACE | OBJ_GOAL_FLAG;
        break;
      case 'p':
        t = OBJ_MAN;
        break;
      case 'P':
        t = OBJ_MAN | OBJ_GOAL_FLAG;
        break;
      case '\n':
        x = 0;
        ++y;
...
void State::draw() const {
  for (int y = 0; y < mHeight; ++y) {
    for (int x = 0; x < mWidth; ++x) {
      switch (mObjects(x, y)) {
        case (OBJ_SPACE | OBJ_GOAL_FLAG):
          cout << '.';
          break;
        case (OBJ_WALL | OBJ_GOAL_FLAG):
          cout << '#';
          break;
        case (OBJ_BLOCK | OBJ_GOAL_FLAG):
          cout << 'O';
          break;
        case (OBJ_MAN | OBJ_GOAL_FLAG):
          cout << 'P';
          break;
        case OBJ_SPACE:
          cout << ' ';
...
void State::update(char input) {
  ...
  for (y = 0; y < h; ++y) {
    for (x = 0; x < w; ++x) {
      if ((o(x, y) & ~OBJ_GOAL_FLAG) ==
          OBJ_MAN) {  // ゴールフラグ以外を取り出すための& ~OBJ_GOAL_FLAG
        found = true;
        break;
      }
    }
  ...
  if ((o(tx, ty) & ~OBJ_GOAL_FLAG) == OBJ_SPACE) {
    o(tx, ty) =
        (o(tx, ty) & OBJ_GOAL_FLAG) |
        OBJ_MAN;  // ゴールフラグを保存するためにこんな面倒なことが必要。以下もほぼ同じ
