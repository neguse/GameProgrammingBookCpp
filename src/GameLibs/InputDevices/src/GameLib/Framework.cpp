...
#include "GameLib/FontTextureGenerated.h"
...
class Impl {
 public:
  ...
  void start(void* windowHandle) {
    ...
    // 描画初期化
    Graphics::Manager::create(windowHandle, mWidth * 2, mHeight * 2,
                              mFullScreen, mVSync, mAntiAlias);
    // 文字描画
    ...
    mDebugFont =
        Scene::Font::create(fontDataArray, &textureSize, 1, &(chars[0]), 158);
    mDebugStringRenderer =
        Scene::StringRenderer::create(2048, 128);  // こんなもんで足りるだろう？
  ...
  void postUpdate() {
    ...
    for (int i = 0; i < mWidth; ++i) {
      ...
      ASSERT(mVideoMemoryWithPadding[mWidth * (mHeight + 1) + i] ==
                 MAGIC_NUMBER &&
             "VRAM RANGE ERROR!");
    }
    // VRAMコピー
...
void Framework::Configuration::setArchiveName( int index, const char* name ){
        ASSERT( gImpl->mArchiveNames && "you must call setArchiveNumber() before
this call." ); ASSERT( index >= 0 && index < gImpl->mArchiveNumber );
...
void Framework::Configuration::setLoadMode( LoadMode lm ){
        ...
        switch ( lm ){
                case LOAD_ARCHIVE_FIRST: am = Manager::MODE_ARCHIVE_FIRST;
break; case LOAD_DIRECT_FIRST: am = Manager::MODE_DIRECT_FIRST; break; case
LOAD_ARCHIVE_ONLY: am = Manager::MODE_ARCHIVE_ONLY; break; default: ASSERT(
false ); break;
        }
        gImpl->mLoadMode = am;
}
...
StandardInput& StandardInput::operator>>( char& a ){
        ...
        while ( true ){ //無限ループ
                ...
                float pointerScale;
                Vector2 pointerOffset;
                Graphics::Manager::getInstance().getPointerModifier(
&pointerScale, &pointerOffset ); Input::Manager::getInstance().update(
pointerScale, pointerOffset );
                //ウィンドウから終了要請を受けているようなら抜ける
                bool altF4 = kb.isOn( Input::Keyboard::KEY_ALT ) && kb.isOn(
Input::Keyboard::KEY_F4 ); if ( altF4 ){ Framework::getInstance().requestEnd();
                        break;
                }
                mPrevEnterOn = enterOn;
        }
