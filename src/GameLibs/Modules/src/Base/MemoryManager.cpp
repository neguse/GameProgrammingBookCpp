...
const U4 HEAP_REGION_SIZE_BIT =
    24;  // OSから一気にせしめる領域サイズ(例えば20で1MB, 24で16MB)
const U4 ALIGN =
    8;  // アラインサイズ4,8,16あたりから選ぼう。4を選ぶ時はdoubleがないことを保障すること。
// 導出定数群
const U4 HEAP_REGION_SIZE = (1 << HEAP_REGION_SIZE_BIT);
const U4 TABLE_NUMBER =
    (HEAP_REGION_SIZE_BIT - 3) *
    4;  // 空きリスト箱の数(4ごとに倍になるのでサイズビット数の4倍だが、4,8,12,16と始まるため、いろいろあって-3)
...
template <class T>
inline T* cast(void* p, int offsetInByte) {
  return reinterpret_cast<T*>(ptr(p) + offsetInByte);
}

// アドレス差ゲット
template <class A, class B>
inline U4 diff(A* p0, B* p1) {
...
void* allocateAlignedMemory(void** originalPointer, U4 size, U4 alignSize) {
  ...
  STRONG_ASSERT((commited == aligned) &&
                "MemoryManager : Unexpected Error! VirtualAlloc() returned "
                "invalid value.");
...
void* allocateMemory(U4 size) {
  ...
  void* p =
      VirtualAlloc(NULL, alignedSize, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
...
void deallocateMemory(void* p) {
  ...
  STRONG_ASSERT(
      (succeeded != 0) &&
      "MemoryManager : Deallocation failed! The pointer must be invalid.");
}
#endif

struct Lock {
  Lock() {
  }  // デフォルトコンストラクタは何もしない。グローバルに置かれる事態を想定する。
...
class Heap {
 public:
  Heap(void* reserved) : mReserved(reserved), mNext(0), mLock(false) {
    ...
    U4 body = align(sizeof(Heap) + VACANT_HEADER_SIZE - 4, ALIGN) -
              VACANT_HEADER_SIZE;
    U4 bodySize =
        HEAP_REGION_SIZE - body -
        8;  // 8バイト減らすのは、次のブロックに無理やり書き込む4バイトと、フラグをセットする次の4倍バイトの分。

    // リンク初期化
    for (int i = 0; i < TABLE_NUMBER; ++i) {
      U4 headAddr = diff(mHeads[i], this);
      U4* head = mHeads[i];
      head[PREV_SIZE] = head[SIZE] =
          0xffffffff;  // 誰も見ないところなので死にそうな数を入れておく。
  ...
  static Heap* create() {
    void* reserved;
    Heap* r = reinterpret_cast<Heap*>(
        allocateAlignedMemory(&reserved, HEAP_REGION_SIZE, HEAP_REGION_SIZE));
  ...
  void removeFromList(U4 addr) {
    if (getSize(addr) >=
        VACANT_HEADER_SIZE) {  // サイズが足りてないものは遅延開放中。
  ...
  void insertToList(U4 addr, U4 size) {
    ...
    U4 index = getHeadIndex(
        size - OCCUPYED_HEADER_SIZE +
        4);  // ヘッダ分引いて、4バイトのブロックに食い込むので、このサイズが正味。
    // 新しいリストに差し込む。先頭に挿入
...
#endif
    void* r = 0;
    U4 headIndex = getHeadIndex(size) +
                   1;  // このサイズが十分に入る箱が欲しいので、+1する必要がある
  ...
  void deallocate(void* p) {
    ...
    STRONG_ASSERT(
        !(nextSizeWithFlagDebug &
          FLAG_PREV_EMPTY));  // 使ってるんだから空いてるわけないです。
    ...
    if (prevIsEmpty) {  // 前が空いてる。
      ...
      if (nextIsEmpty) {  // 次も空いてる
        ...
        if (size >=
            VACANT_HEADER_SIZE) {  // リストにつなげる状況でだけつなぐ。足りないときは隣が解放されるのを待つ。
          insertToList(addr, size);
        }
        setEmptyFlag(addr);
        // 次のブロックに空いていることを伝える
  ...
  void check() {
    ...
    */
    U4 begin = align(sizeof(Heap) + VACANT_HEADER_SIZE - 4, ALIGN) -
               VACANT_HEADER_SIZE;
    U4 current = begin;
    U4 prevSizeCheck = 0;
    bool prevIsEmpty = false;
    U4 end = HEAP_REGION_SIZE -
             OCCUPYED_HEADER_SIZE;  // ここより前だろ。でないとヘッダがはいらん
    while (current < end) {
  ...
  enum Header {
    PREV_SIZE =
        0,  // こいつは前のブロックが空いてる時だけつかう。それ以外の時は壊されている。
    SIZE = 1,
#ifdef USE_DEBUG_INFO
...
class Impl {
 public:
  ...
  void* allocate(size_t sizeOrig) {
    ...
    if (size > MAX_NORMAL_BLOCK_SIZE) {
      ...
      Heap* current =
          mHead;  // ロック不要。0なら0で結構。0以外ならどれでもかまわん。
      while (current) {
        if (current->tryLock()) {
#ifdef USE_DEBUG_INFO
  ...
  void write(const char* filename) {
    ...
    if (filename) {
      ...
      STRONG_ASSERT((e == 0) &&
                    "MemoryManager::write() : can't open output file.");
    } else {
      ::OutputDebugStringA("\n");
    }
    char str
        [8192];  // 大過剰に取るよ。ここでケチっても何の得もないからな。本当sprintfはダメだなと思う。
    int l = 0;
    l += sprintf_s(str + l, 8192 - l, "[ MemoryManager::write() ]\n");
    l += sprintf_s(str + l, 8192 - l, "totalSize = %d ( %x ) : %.2fMB\n",
                   mTotalSize, mTotalSize,
                   static_cast<float>(mTotalSize) / 1024.f / 1024.f);
 ...
 private:
  ...
  void deallocateLb(void* p) {
    ...
    U4 alignedHeaderSize = align(LB_HEADER_SIZE, ALIGN);
    origP -= alignedHeaderSize -
             LB_HEADER_SIZE;  // アラインでずらしたなら、返却アドレスもずらす
